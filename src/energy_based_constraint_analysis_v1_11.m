...
%Variables
    ...
    %User desire input
rho_turn = 0.905;
velocity_turn = 56.6; %Gotten from Cirrus SR 20 in m/s
%Sub-formulas
q_turn = 0.5 .* rho_turn .* velocity_turn.^2;
C_D0_turn = ;
C_D_turn = ;
%Thrust to Weight calculation
(Beta ./ alpha) .* (((K_1 .* Beta .* n.^2) ./ q_turn) .* ...
    (Weight_TO ./ Wing_area) + K_2 .* n + C_D0_turn .* ...
    (Beta ./ q_turn) .* (1 ./ (Weight_TO ./ Wing_area)))
