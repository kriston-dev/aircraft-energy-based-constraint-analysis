...
q_climb = 0.5 .* user_aircraft_input.rho_climb .* velocity_climb.^2;
...
q_cruise = 0.5 .* user_aircraft_input.rho_cruise .* velocity_cruise.^2;
...
velocity_stall_straight = sqrt((2 .* Wing_loading) ...
    ./ (user_aircraft_input.rho_turn .* C_Lmax_TO)); %A straight line of aircraft stall velocity
...
for refine_loop = 1:1000
    ...
    velocity_stall_turn = sqrt((2 * Wing_loading .* n) ...
        ./ (user_aircraft_input.rho_turn .* C_Lmax_TO));
...
q_accel = 0.5 .* user_aircraft_input.rho_horiz_accel .* velocity_accel.^2;
...
%Ask user
user_aircraft_input.altitude_TO = input("Takeoff altitude: ");
user_aircraft_input.altitude_climb = input("climb altitude (m): ");
user_aircraft_input.altitude_cruise = input("cruise altitude (m): ");
user_aircraft_input.altitude_turn = input("turn altitude (m): ");
user_aircraft_input.altitude_horiz_accel = input("Horizontal acceleration altitude (m): ");
user_aircraft_input.altitude_approach = input("approach altitude (m): ");

%Calculate
user_aircraft_input.rho_TO = altitude_find_rho(user_aircraft_input.altitude_TO);
user_aircraft_input.rho_climb = altitude_find_rho(user_aircraft_input.altitude_climb);
user_aircraft_input.rho_cruise = altitude_find_rho(user_aircraft_input.altitude_cruise);
user_aircraft_input.rho_turn = altitude_find_rho(user_aircraft_input.altitude_turn);
user_aircraft_input.rho_horiz_accel = altitude_find_rho(user_aircraft_input.altitude_horiz_accel);
user_aircraft_input.rho_approach = altitude_find_rho(user_aircraft_input.altitude_approach);
