...
while true
    switch currState
        ...
        case Aircraft_constraint_states.FIND_TW
            break
        
        case Aircraft_constraint_states.GET_AIRCRAFT_DATA
            get_user_aircraft_design_inputs();
    end
end
...
function rho = altitude_find_rho(altitude_user)
temp_SL = 288.15;
temp_lapse_rate = 0.0065;
air_gas = 287.05;

rho = rho_SL .* (1 - (temp_lapse_rate .* altitude_user) ./ temp_SL) ...
...
if strcmp(user_path, 'Find T/W')
    currState = Aircraft_constraint_states.FIND_TW;
else
    disp("Invalid choice");
    currState = Aircraft_constraint_states.MAIN_MENU;
end
