...
%A more specific version of Beta could have bee the mass while climbing ./
%Mass_TO

Thrust_to_Weight_climb = Beta/alpha .* (((K_1 .* n.^2 .* Beta) ./ q_climb) .* (Weight_TO ./ Wing_area) + K_2 .* n + C_D0 ./ ((Beta ./ q_climb) .* Wing_loading) + rate_of_climb ./ velocity_climb);
...
rho_cruise = 1.2133; % Represents 100 meters of altitude

% sub-formulas for constraint

C_Lcruise = 2 .*Weight_TO./(rho_cruise .* Cruise_velocity.^2 .* Wing_area);

c_D0_cruise = C_D0 + (K_1.*C_Lcruise.^2) + K_2.*C_Lcruise;

%C_Dcruise = C_D0 + (K_1.*C_Lcruise.^2) + K_2.*C_Lcruise;


q_cruise = 0.5 .* rho_cruise .* velocity_climb.^2;

% Calculate the thrust-to-weight ratio for cruise
Thrust_to_Weight_cruise = (q_cruise * c_D0_cruise) ./ (Weight_TO ./ Wing_area) + K_1 .* ((Weight_TO ./ Wing_area) ./ q_cruise) + K_2;
...
hold off; %stops the plots from being in the hold mode thus future  plots can ovveride
