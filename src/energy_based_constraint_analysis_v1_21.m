...
while true
    switch currState
        ...
        case Aircraft_constraint_states.GET_TW_AIRCRAFT_DATA
            get_user_aircraft_design_inputs(rho_SL, Gravity);
            currState = Aircraft_constraint_states.CALCULATE_TW;

        case Aircraft_constraint_states.CALCULATE_TW %Do NOT RUN it will slow computer and take logn time, it keeps runnnig claculation
            TW = found_TW(user_aircraft_input, Gravity);
            currState = Aircraft_constraint_states.Display_TW;

        case Aircraft_constraint_states.DISPLAY_TW
            graph_TW(TW);
    end
end
...
if strcmp(user_path, 'Find T/W')
    ...
    disp("current state is GET_TW_AIRCRAFT_DATA")
else
    disp("Invalid choice");
    currState = Aircraft_constraint_states.MAIN_MENU;
end
...
function user_aircraft_input = get_user_aircraft_design_inputs(rho_SL, Gravity)

%user_aircraft_input.
%Will add more once organized all the user inputs and can add

%user_aircraft_input.Span = input("Span: ");

altitude_TO = input("Takeoff altitude (m): ");
altitude_climb = input("climb altitude (m): ");
altitude_cruise = input("cruise altitude (m): ");
altitude_turn = input("turn altitude (m): ");
altitude_horiz_accel = input("Horizontal acceleration altitude (m): ");
altitude_approach = input("approach altitude (m): ");

user_aircraft_input.rho_TO = altitude_find_rho(rho_SL, Gravity, altitude_TO);
user_aircraft_input.rho_climb = altitude_find_rho(rho_SL, Gravity, altitude_climb);
user_aircraft_input.rho_cruise = altitude_find_rho(rho_SL, Gravity, altitude_cruise);
user_aircraft_input.rho_turn = altitude_find_rho(rho_SL, Gravity, altitude_turn);
user_aircraft_input.rho_horiz_accel = altitude_find_rho(rho_SL, Gravity, altitude_horiz_accel);
user_aircraft_input.rho_approach = altitude_find_rho(rho_SL, Gravity, altitude_approach);

end



function TW = found_TW(user_aircraft_input, Gravity)
...
Propeller_efficiency_TO = 0.75; % assume based on propeller
    ...
    %Velocity formulas
Velocity_stall = sqrt((2 .* Weight_TO) ./ (user_aircraft_input.rho_TO .* Wing_area .* C_Lmax_TO));
...
TW.TO = Thrust_to_Weight_TO;
TW.climb = Thrust_to_Weight_climb;
TW.cruise = Thrust_to_Weight_cruise;
TW.turn = Thrust_to_Weight_turn;
TW.horiz_accel = Thrust_to_Weight_horiz_accel;

TW.Wing_loading = Wing_loading;
TW.approach = Wing_loading_approach;

end



function rho = altitude_find_rho(rho_SL, Gravity, altitude_TO)
temp_SL = 288.15;
...
function graph_TW(TW)
plot(Wing_loading, TW.Thrust_to_Weight_TO, '-');
plot(Wing_loading, TW.Thrust_to_Weight_climb, '-');
plot(Wing_loading, TW.Thrust_to_Weight_cruise, '-');
plot(Wing_loading, TW.Thrust_to_Weight_turn, '-');
plot(Wing_loading, TW.Thrust_to_Weight_horiz_accel, '-');

TW.Wing_loading = Wing_loading;
xline(TW.Wing_loading_approach, '-');
